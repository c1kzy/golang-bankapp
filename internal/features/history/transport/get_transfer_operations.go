...
func (h *HistoryHTTPHandler) GetTransferOperations(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	transactions, err := h.historyService.GetTransferOperations(ctx, userID)
	if err != nil {
		responseHandler.ErrorResponse(err, "unable to get transfer operations")
